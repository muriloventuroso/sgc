...
pdfmetrics.registerFont(
    TTFont('Roboto', 'static/fonts/RobotoSlab-Regular.ttf'))
ANNOT_KEY = '/Annots'
ANNOT_FIELD_KEY = '/T'
ANNOT_VAL_KEY = '/V'
ANNOT_RECT_KEY = '/Rect'
...
class TransactionSheetPdf(object):
    def __init__(self, month, balance, congregation_id, checks=[], data_off={}):
        ...
        self.end_date = datetime.combine(
            self.start_date.replace(day=self.last_day), time.max)
        if not self.balance:
    ...
    def set_page1(self):
        self.pdf.drawImage('financial/templates/pdf/S26-1.png',
    ...
    def set_page2(self):
        ...
        self.pdf.showPage()
        self.pdf.setFont("Roboto", 7)
        self.pdf.drawImage('financial/templates/pdf/S26-2.png',
                           0, 0, width=600, height=850)
    ...
    def generate_header(self):
        ...
        self.pdf.drawString(25, 795, self.congregation.name +
                            " / " + self.congregation.circuit)
                ...
                if sub_transaction.description:
                    ...
                    if sub_transaction.tt == 'R':
                        if sub_transaction.td == "I":
                            x = 316 - \
                                stringWidth2(self.pdf, value, "Roboto", 9, 1)
                            self.pdf.drawString(x, y, value)
    ...
    def generate_confrontation(self):
        ...
        self.pdf.drawString(
            300, 772, datetime.now().strftime('%d de %B de %Y'))

        item1 = float(self.balance) + self.sum_c_i - \
            self.sum_c_o + self.sum_r_i - self.sum_r_o

        self.pdf.setFont("Roboto", 9)
        self.pdf.drawString(
            250, 730, "{0:.2f}".format(item1).replace('.', ','))
        item2 = 0.0
        if 'deposits' in self.data_off and self.data_off["deposits"]:
            item2 = float(self.data_off["deposits"])
            self.pdf.drawString(
                250, 705, "{0:.2f}".format(item2).replace('.', ','))
        item3 = 0.0
        if 'bank_fees' in self.data_off and self.data_off["bank_fees"]:
            item3 = float(self.data_off["bank_fees"])
            self.pdf.drawString(
                250, 690, "{0:.2f}".format(item3).replace('.', ','))
        item4 = item1 + item2 + item3
        self.pdf.drawString(
            250, 676, "{0:.2f}".format(item4).replace('.', ','))
        ...
        self.pdf.drawString(
            250, 434, "{0:.2f}".format(item9).replace('.', ','))

        self.pdf.drawString(170, 357, self.end_date.strftime('%d de %B de %Y'))
        self.pdf.drawString(190, 320, "0,00")
        self.pdf.drawString(190, 305, "{0:.2f}".format(
            self.sum_r_i).replace('.', ','))
        self.pdf.drawString(190, 290, "{0:.2f}".format(
            self.sum_r_o).replace('.', ','))
        self.pdf.drawString(250, 275, "{0:.2f}".format(
            self.sum_r_i - self.sum_r_o).replace('.', ','))

        self.pdf.drawString(190, 238, str(self.balance).replace('.', ','))
        self.pdf.drawString(190, 223, "{0:.2f}".format(
            self.sum_c_i - sum_off).replace('.', ','))
        self.pdf.drawString(190, 208, "{0:.2f}".format(
            self.sum_c_o).replace('.', ','))
        ...
        self.pdf.drawString(250, 88, "{0:.2f}".format(
            float(self.balance) + self.sum_c_i - self.sum_c_o +
            self.sum_r_i - self.sum_r_o - sum_off
        ).replace('.', ','))
...
class MonthlyReportPdf(object):
    ...
    def set_page1(self):
        self.pdf.drawImage('financial/templates/pdf/S30-1.png',
                           0, 0, width=600, height=850)
    ...
    def set_page2(self):
        ...
        self.pdf.showPage()
        self.pdf.setFont("Roboto", 7)
        self.pdf.drawImage('financial/templates/pdf/S30-2.png',
                           0, 0, width=600, height=850)
    ...
    def generate_header(self):
        self.pdf.setFont("Roboto", 11)
        self.pdf.drawString(120, 706, self.congregation.name +
                            " / " + self.congregation.circuit)
        self.pdf.drawString(420, 706, self.start_date.strftime(
            "%B").upper() + " / " + self.start_date.strftime("%Y"))
    ...
    def generate_transactions(self):
            ...
            if transaction.tc in ('C', 'O', 'F', 'CE', 'OE'):
                if transaction.category_id:
                    if str(transaction.category_id) not in receipt_cats:
                        receipt_cats[str(transaction.category_id)] = {
                ...
                if transaction.tc == "O":
                    ...
                    self.sum_construction_subsidiary += float(
                        transaction.value)
        ...
        self.pdf.drawString(355, 618, "{0:.2f}".format(
            self.sum_receipts_eletronic).replace('.', ','))
        y = 603
        for key, value in receipt_cats.items():
            self.pdf.drawString(60, y, value['name'])
            self.pdf.drawString(355, y, "{0:.2f}".format(
                value['value']).replace('.', ','))
            y -= 15
        self.b = self.sum_receipts + self.sum_receipts_eletronic + self.sum_specific_objective
        self.pdf.drawString(
            420, 577, "{0:.2f}".format(self.b).replace('.', ','))

        self.pdf.drawString(355, 545, "{0:.2f}".format(
            self.sum_world_wide).replace('.', ','))
        self.pdf.drawString(59, 529, str(
            _("Donates to Construction of the Subsidiary")))
        self.pdf.drawString(355, 530, "{0:.2f}".format(
            self.sum_construction_subsidiary).replace('.', ','))
        self.c = self.sum_world_wide + self.sum_construction_subsidiary
        self.pdf.drawString(
            420, 498, "{0:.2f}".format(self.c).replace('.', ','))
        self.d = self.b + self.c
        self.pdf.drawString(
            494, 482, "{0:.2f}".format(self.d).replace('.', ','))
        ...
        self.pdf.drawString(
            420, 305, "{0:.2f}".format(self.e).replace('.', ','))

        self.pdf.drawString(355, 278, "{0:.2f}".format(
            self.sum_world_wide).replace('.', ','))
        self.pdf.drawString(59, 263, str(
            _("Donates to Construction of the Subsidiary")))
        self.pdf.drawString(355, 263, "{0:.2f}".format(
            self.sum_construction_subsidiary).replace('.', ','))
        self.f = self.sum_world_wide + self.sum_construction_subsidiary

        self.pdf.drawString(
            420, 235, "{0:.2f}".format(self.f).replace('.', ','))

        self.g = self.e + self.f

        self.pdf.drawString(
            492, 222, "{0:.2f}".format(self.g).replace('.', ','))

        self.h = self.d - self.g

        self.pdf.drawString(
            492, 205, "{0:.2f}".format(self.h).replace('.', ','))
        self.i = self.a + self.h

        self.pdf.drawString(
            492, 185, "{0:.2f}".format(self.i).replace('.', ','))

        self.pdf.drawString(
            492, 95, "{0:.2f}".format(self.i).replace('.', ','))
        self.pdf.setFont("Roboto", 11)
    ...
    def generante_announcement(self):
        ...
        self.pdf.drawString(
            495, 681, "{0:.2f}".format(self.b).replace('.', ','))
        self.pdf.drawString(
            360, 650, "{0:.2f}".format(self.e).replace('.', ','))
        self.pdf.drawString(
            125, 617, "{0:.2f}".format(self.i).replace('.', ','))
        self.pdf.drawString(
            490, 617, "{0:.2f}".format(self.f).replace('.', ','))
    ...
    def generate_summary(self):
        ...
        for transaction in self.transactions:
            ...
            if tc:
                if tc not in transactions_tc:
                    transactions_tc[tc] = TransactionContent(
                        tc=tc, count=0, value=0)
        ...
        summary.date = datetime.now().replace(
            month=self.start_date.month).replace(year=self.start_date.year)
