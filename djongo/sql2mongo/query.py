...
class UpdateQuery(DMLQuery):
    ...
    def execute(self):
        ...
        logger.debug(
            f'update_many: {self.result.modified_count}, matched: {self.result.matched_count}')
...
class InsertQuery(DMLQuery):
        ...
        if collection not in self.connection_properties.cached_collections:
            if self.connection_properties.enforce_schema:
                raise MigrationError(
                    f'Table {collection} does not exist in database')
    ...
    def _columns(self, statement: SQLStatement):
        tok = statement.next()
        self._cols = [
            token.column for token in SQLToken.tokens2sql(tok[1], self)]
...
class AlterQuery(DDLQuery):
    ...
    def _drop_column(self):
        ...
        self.db['__schema__'].update_many(
            {'name': self.left_table},
            {
                '$unset': {
        ...
        for col in SQLColumnDef.sql2col_defs(tok.value):
            if isinstance(col, SQLColumnConstraint):
                ...
                if SQLColumnDef.primarykey in col.col_constraints:
                    self.db[table].create_index(
                        field, unique=True, name='__primary_key__')
...
class Query:
    ...
    def parse(self):
        ...
        def check_conditions(item):
            if not getattr(self, 'is_where', False):
                self.is_where = getattr(item, 'value', None) == 'WHERE' and getattr(
                    item, 'ttype', None) == Keyword
            ...
            next_token = parent.token_next(
                index_of_precedence, skip_ws=True, skip_cm=True)[1]
        ...
        def parse_where(where_item):
            ...
            for identifier_token_list_index in range(1, len(identifier_token_list), 2):
                identifier_token_list.insert(
                    identifier_token_list_index, Token(ttype=Punctuation, value='.'))
            return Comparison([
        ...
        def from_part(token, token_list):
            ...
            if hasattr(token, 'tokens'):
                ...
                for new_token in getattr(token, 'tokens', []):
                    to_append_token_list.append(
                        from_part(new_token, token_list))
    ...
    def _alter(self, sm):
        try:
            query = AlterQuery(
                self.db, self.connection_properties, sm, self._params)
        except SQLDecodeError:
            logger.warning(
                'Not implemented alter command for SQL {}'.format(self._sql))
            raise
        else:
            query.execute()
            return query
    ...
    def _insert(self, sm):
        query = InsertQuery(
            self, self.db, self.connection_properties, sm, self._params)
        query.execute()
        return query
